...
- (void)setDelayTime:(float)delayTime {
    AudioUnitSetParameter(_auDelay, kDelayParam_DelayTime, kAudioUnitScope_Global, 0, delayTime, 0);
}

- (float)delayTime {
    float delayTime = 0;
    AudioUnitGetParameter(_auDelay, kDelayParam_DelayTime, kAudioUnitScope_Global, 0, &delayTime);
    return delayTime;
}
...
- (BOOL)setupNodes {
    ...
    delayDescription.componentFlags = 0;
    delayDescription.componentFlagsMask = 0;
    ...
    __Require_noErr(AUGraphAddNode(_auGraph, &delayDescription, &_nodeDelay), fail);
...
- (BOOL)connectNodes {
    __Require_noErr(AUGraphConnectNodeInput(_auGraph, _nodeFilePlayer, 0, _nodeDelay, 0), fail);
    __Require_noErr(AUGraphConnectNodeInput(_auGraph, _nodeDelay, 0, _nodeRemoteIO, 0), fail);
    return YES;
fail:
    return NO;
}
...
- (BOOL)prepareAUs {
    ...
    __Require_noErr(AUGraphNodeInfo(_auGraph, _nodeDelay, NULL, &_auDelay), fail);
    return YES;
fail:
    return NO;
}
...
- (BOOL)resetGraph {
    ...
    __Require_noErr(AudioUnitReset(_auDelay, kAudioUnitScope_Global, 0), fail);
    __Require_Quiet([self adjustStartupTime], fail);
    return YES;
fail:
    return NO;
